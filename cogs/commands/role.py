...
class Server(commands.Cog):
    ...
    async def add_role(self, *, role: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            ...
            await member.add_roles(role, reason="Krypton | Role Added ")

    async def remove_role(self, *, role: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            role = discord.Object(id=int(role))
            await member.remove_roles(role, reason="Krypton | Role Removed")
    ...
    async def _owner(self, context: Context, member: discord.Member) -> None:
        if data := getConfig(context.guild.id):
            ...
            hacker = discord.Embed(
                description=
                f"<:GreenTick:1018174649198202990> | Successfully Given <@&{role}> To {member.mention}",
                color=0x2f3136)
            ...
            hacker1 = discord.Embed(
                description=
                f"<:Krypton_claim:1038130143547375667> | Owner role is not setuped in {context.guild.name}",
                color=0x2f3136)
    ...
    async def _coowner(self, context: Context, member: discord.Member) -> None:
        if data := getConfig(context.guild.id):
            ...
            hacker1 = discord.Embed(
                description=
                f"<:Krypton_claim:1038130143547375667> | Co Owner role is not setuped in {context.guild.name}",
                color=0x2f3136)
            hacker1.set_author(name=f"{context.author}",
                               icon_url=f"{context.author.avatar}")
            hacker1.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def _headadmin(self, context: Context,
        ...
        if data := getConfig(context.guild.id):
            ...
            hacker1 = discord.Embed(
                description=
                f"<:Krypton_claim:1038130143547375667> | Head Admin role is not setuped in {context.guild.name}",
                color=0x2f3136)
            hacker1.set_author(name=f"{context.author}",
                               icon_url=f"{context.author.avatar}")
            hacker1.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def _admin(self, context: Context, member: discord.Member) -> None:
        if data := getConfig(context.guild.id):
            ...
            hacker1 = discord.Embed(
                description=
                f"<:Krypton_claim:1038130143547375667> | Admin role is not setuped in {context.guild.name}",
                color=0x2f3136)
            hacker1.set_author(name=f"{context.author}",
                               icon_url=f"{context.author.avatar}")
            hacker1.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def _girladmin(self, context: Context,
        ...
        if data := getConfig(context.guild.id):
            ...
            hacker1 = discord.Embed(
                description=
                f"<:Krypton_claim:1038130143547375667> | Girl Admin role is not setuped in {context.guild.name}",
                color=0x2f3136)
            hacker1.set_author(name=f"{context.author}",
                               icon_url=f"{context.author.avatar}")
            hacker1.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def _headmod(self, context: Context, member: discord.Member) -> None:
        if data := getConfig(context.guild.id):
            ...
            hacker1 = discord.Embed(
                description=
                f"<:Krypton_claim:1038130143547375667> | Head Mod role is not setuped in {context.guild.name}",
                color=0x2f3136)
            hacker1.set_author(name=f"{context.author}",
                               icon_url=f"{context.author.avatar}")
            hacker1.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def _mod(self, context: Context, member: discord.Member) -> None:
        if data := getConfig(context.guild.id):
            ...
            hacker1 = discord.Embed(
                description=
                f"<:Krypton_claim:1038130143547375667> | Mod role is not setuped in {context.guild.name}",
                color=0x2f3136)
            hacker1.set_author(name=f"{context.author}",
                               icon_url=f"{context.author.avatar}")
            hacker1.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def _girlmod(self, context: Context, member: discord.Member) -> None:
        if data := getConfig(context.guild.id):
            ...
            hacker1 = discord.Embed(
                description=
                f"<:Krypton_claim:1038130143547375667> | Girl Mod role is not setuped in {context.guild.name}",
                color=0x2f3136)
            hacker1.set_author(name=f"{context.author}",
                               icon_url=f"{context.author.avatar}")
            hacker1.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def owner(self, context: Context, role: discord.Role) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            if data := getConfig(context.guild.id):
                ...
                hacker = discord.Embed(
                    description=
                    f"<:GreenTick:1018174649198202990> | Successfully Setuped `Owner` Role To {role.mention}",
                    color=0x2f3136)
                hacker.set_author(name=f"{context.author}",
                                  icon_url=f"{context.author.avatar}")
                hacker.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def coowner(self, context: Context, role: discord.Role) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            if data := getConfig(context.guild.id):
                ...
                hacker = discord.Embed(
                    description=
                    f"<:GreenTick:1018174649198202990> | Successfully Setuped `Co Owner` Role To {role.mention}",
                    color=0x2f3136)
                hacker.set_author(name=f"{context.author}",
                                  icon_url=f"{context.author.avatar}")
                hacker.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def headadmin(self, context: Context, role: discord.Role) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            if data := getConfig(context.guild.id):
                ...
                hacker = discord.Embed(
                    description=
                    f"<:GreenTick:1018174649198202990> | Successfully Setuped `Head Admin` Role To {role.mention}",
                    color=0x2f3136)
                hacker.set_author(name=f"{context.author}",
                                  icon_url=f"{context.author.avatar}")
                hacker.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def admin(self, context: Context, role: discord.Role) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            if data := getConfig(context.guild.id):
                ...
                hacker = discord.Embed(
                    description=
                    f"<:GreenTick:1018174649198202990> | Successfully Setuped `Admin` Role To {role.mention}",
                    color=0x2f3136)
                hacker.set_author(name=f"{context.author}",
                                  icon_url=f"{context.author.avatar}")
                hacker.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def headmod(self, context: Context, role: discord.Role) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            if data := getConfig(context.guild.id):
                ...
                hacker = discord.Embed(
                    description=
                    f"<:GreenTick:1018174649198202990> | Successfully Setuped `Head Mod` Role To {role.mention}",
                    color=0x2f3136)
                hacker.set_author(name=f"{context.author}",
                                  icon_url=f"{context.author.avatar}")
                hacker.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def mod(self, context: Context, role: discord.Role) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            if data := getConfig(context.guild.id):
                ...
                hacker = discord.Embed(
                    description=
                    f"<:GreenTick:1018174649198202990> | Successfully Setuped `Mod` Role To {role.mention}",
                    color=0x2f3136)
                hacker.set_author(name=f"{context.author}",
                                  icon_url=f"{context.author.avatar}")
                hacker.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def girladmin(self, context: Context, role: discord.Role) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            if data := getConfig(context.guild.id):
                ...
                hacker = discord.Embed(
                    description=
                    f"<:GreenTick:1018174649198202990> | Successfully Setuped `Girl Admin` Role To {role.mention}",
                    color=0x2f3136)
                hacker.set_author(name=f"{context.author}",
                                  icon_url=f"{context.author.avatar}")
                hacker.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def girlmod(self, context: Context, role: discord.Role) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            if data := getConfig(context.guild.id):
                ...
                hacker = discord.Embed(
                    description=
                    f"<:GreenTick:1018174649198202990> | Successfully Setuped `Girl Mod` Role To {role.mention}",
                    color=0x2f3136)
                hacker.set_author(name=f"{context.author}",
                                  icon_url=f"{context.author.avatar}")
                hacker.set_thumbnail(url=f"{context.author.avatar}")
    ...
    async def rsta(self, context: Context) -> None:
        if data := getConfig(context.guild.id):
            ...
            embed.add_field(
                ...
                inline=False)
            embed.add_field(
                name="<a:im_arrowr:1038029121881636884> Owner Role:",
                value=f"{own}",
                inline=False)
            embed.add_field(
                name="<a:im_arrowr:1038029121881636884> Co Owner Role:",
                value=f"{coown}",
                inline=False)
            embed.add_field(
                name="<a:im_arrowr:1038029121881636884> Head Admin Role:",
                value=f"{headmin}",
                inline=False)
            embed.add_field(
                name="<a:im_arrowr:1038029121881636884> Admin Role:",
                value=f"{adminss}",
                inline=False)
            embed.add_field(
                name="<a:im_arrowr:1038029121881636884> Girl Admin Role:",
                value=f"{gadminss}",
                inline=False)
            embed.add_field(
                name="<a:im_arrowr:1038029121881636884> Head Mod Role:",
                value=f"{hmod}",
                inline=False)
            embed.add_field(name="<a:im_arrowr:1038029121881636884> Mod Role:",
                            value=f"{mod}",
                            inline=False)
            embed.add_field(
                name="<a:im_arrowr:1038029121881636884> Girl Mod Role:",
                value=f"{gmod}",
                inline=False)
    ...
    async def _create(self, ctx, name, *, message):
        ...
        if str(ctx.guild.id) in autoresponse:
            ...
            hacker1 = discord.Embed(
                title="Krypton",
                description=
                f"<a:black_Krypton:1002204507985432666> | Successfully Created Autoresponder in {ctx.guild.name} with the `{name}`",
                color=0x2f3136)
        ...
        if str(ctx.guild.id) in autoresponse:
            if name in autoresponse[str(ctx.guild.id)]:
                ...
                hacker1 = discord.Embed(
                    title="Krypton",
                    description=
                    f"<a:black_Krypton:1002204507985432666> | Successfully Deleted Autoresponder in {ctx.guild.name} with the `{name}`",
                    color=0x2f3136)
                hacker1.set_author(name=f"{ctx.author}",
                                   icon_url=f"{ctx.author.avatar}")
                hacker1.set_thumbnail(url=f"{ctx.author.avatar}")
        ...
        if str(ctx.guild.id) in autoresponse:
            if name in autoresponse[str(ctx.guild.id)]:
                ...
                hacker1 = discord.Embed(
                    title="Krypton",
                    description=
                    f"<a:black_Krypton:1002204507985432666> | Successfully Edited Autoresponder in {ctx.guild.name} with the `{name}`",
                    color=0x2f3136)
                hacker1.set_author(name=f"{ctx.author}",
                                   icon_url=f"{ctx.author.avatar}")
                hacker1.set_thumbnail(url=f"{ctx.author.avatar}")
