    ...
    async def _autorole_humans_add(self, ctx, role: discord.Role):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(rl) == 5:
                embed = discord.Embed(
                    description=
                    f"<:error:1018174714750976030> | You have reached maximum channel limit for autorole humans which is five .",
                    color=0x2f3136)
                await ctx.send(embed=embed)
            else:
    ...
    async def _greet_autodel(self, ctx, *, autodelete_second):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            ...
            hacker = discord.Embed(
                color=0x2f3136,
                description=
                f"<:GreenTick:1029990379623292938> | Successfully updated the welcome autodelete second to {autodelete_second} .\nFrom now welcome message will be deleted after {autodelete_second} .",
                timestamp=ctx.message.created_at)
            hacker.set_author(name=f"{ctx.author.name}",
                              icon_url=f"{ctx.author.avatar}")
            await ctx.send(embed=hacker)
