...
class Automod(Cog):
    ...
    async def _antiraid(self, ctx: commands.Context):
        ...
        spam = data["antiSpam"]
        link = data["antiLink"]
        embed = discord.Embed(title="Krypton | Automod Commands",
                              color=0x2f3136)
        embed.add_field(
            name="<:ok_arrow:1024940412194005052> antispam on/off",
            value=f"Enables/Disables antispam feature\nCurrently Its {spam}",
            inline=False)
        embed.add_field(
            name="<:ok_arrow:1024940412194005052> antilink on/off",
    ...
    async def _antispam(self, ctx: commands.Context, type: str):
        ...
        if ctx.author == ctx.guild.owner or ctx.guild.me.top_role <= ctx.author.top_role:
            ...
            hacker5 = discord.Embed(
                color=0x2f3136,
                title="Krypton Security",
                description=
                f"<:error:1018174714750976030> | Only owner of the server can run this command"
            )
