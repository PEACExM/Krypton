...
class View(discord.ui.View):
    ...
    def add_buttons(self):
        self.startB = Buttons(emoji="⏮️",
                              style=discord.ButtonStyle.grey,
                              command=self.set_page,
                              args=0,
                              ctx=self.ctx)
        self.backB = Buttons(emoji="◀️",
                             style=discord.ButtonStyle.grey,
                             command=self.to_page,
                             args=-1,
                             ctx=self.ctx)
        self.nextB = Buttons(emoji="▶️",
                             style=discord.ButtonStyle.grey,
                             command=self.to_page,
                             args=+1,
                             ctx=self.ctx)
        self.endB = Buttons(emoji="⏭️",
                            style=discord.ButtonStyle.grey,
                            command=self.set_page,
                            args=len(self.options) - 1,
                            ctx=self.ctx)
        self.quitB = Buttons(emoji="⏹️",
                             style=discord.ButtonStyle.grey,
                             command=self.quit,
                             ctx=self.ctx)
    ...
    def gen_embeds(self):
        ...
        options.append(
            discord.SelectOption(label="Home",
                                 emoji='<a:dev:979309038888189982>'))
        ...
        for cog in self.get_cogs():
            if "help_custom" in dir(cog):
                ...
                embed = discord.Embed(title=f"{emoji} {label} Commands",
                                      color=0x01f5b6)
                embed.set_footer(
                    text="Made By ~ Hacker_xD#0007",
                    icon_url=
                    "https://cdn.discordapp.com/avatars/143853929531179008/a_e45a8d0780fbaa7f34214dff69971924.gif?size=2048"
                )
